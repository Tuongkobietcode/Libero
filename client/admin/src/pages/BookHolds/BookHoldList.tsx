import { BookOutlined, CheckCircleOutlined, ClockCircleOutlined, ReloadOutlined, StopOutlined } from '@ant-design/icons';
...
import { BookHoldStatus, type BookHoldListItem } from '../../types/models';
...
function StatusPill({ status }: { status: BookHoldStatus }) {
  return (
    <span className={`inline-flex rounded-lg px-3 py-1 text-xs font-extrabold ${holdStatusClassMap[status]}`}>
      {getBookHoldStatusLabel(status)}
    </span>
  );
}
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_16px_40px_rgba(15,23,42,0.05)]">
      <div className="flex items-center gap-5">
        <span className={`grid h-14 w-14 shrink-0 place-items-center rounded-full text-2xl ring-8 ${toneClasses.icon}`}>
          {icon}
        </span>
  ...
  return (
    <div className="space-y-7">
      <div className="grid gap-5 md:grid-cols-2 2xl:grid-cols-4">
        ...
        <HoldStatCard icon={<ClockCircleOutlined />} label="Hết hạn" value={expiredCountQuery.data?.pagination.totalItems} tone="amber" helper="Quá hạn nhận sách" />
      </div>
      ...
      {holdsQuery.error ? (
        <Alert type="error" showIcon message="Không thể tải danh sách đặt giữ" description={(holdsQuery.error as Error).message} />
      ) : null}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_16px_40px_rgba(15,23,42,0.05)]">
        ...
        {!holdsQuery.isLoading && !items.length ? (
          <div className="px-6 py-16">
            <Empty description="Không có đặt giữ phù hợp." />
          </div>
        ) : null}
