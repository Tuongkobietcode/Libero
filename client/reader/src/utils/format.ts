...
const friendlyErrorMessages: Record<string, string> = {
  ERR_HOLD_LIMIT_REACHED: 'Bạn đã đặt giữ tối đa 3 sách cùng lúc. Hãy nhận hoặc hủy bớt sách đang giữ trước khi đặt giữ thêm.',
};
...
export function extractErrorMessage(error: unknown, fallback: string = 'Yêu cầu thất bại'): string {
  ...
  return formatValidationMessage(apiError) ?? friendlyErrorMessages[apiError?.code ?? ''] ?? apiError?.message ?? maybeAxiosError.message ?? fallback;
}
